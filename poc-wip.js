...
// `;
// let jsonData2Clean = jsonData2.replace(/\n/g, '');
// let df2 = pl.readJSON(jsonData2Clean, { format: "lines" });
// await delay(3000)
// console.log(df2.toString());



const openPrByYearMonth = df1
    .filter(pl.col('event_type').eq('PullRequestEvent')  
            .and(pl.col('state').eq('open')))            
    .withColumn(pl.col('created_at').dt.strptime('%Y-%m-%dT%H:%M:%SZ').alias('created_at_dt')) 
    .groupby([                                           
...
console.log(openPrByYearMonth.toString());